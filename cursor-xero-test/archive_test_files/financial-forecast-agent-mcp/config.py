"""Configuration management for Financial Forecast Agent with MCP Integration."""
...
class Config:
    """Configuration settings for MCP-enabled Financial Forecast Agent."""
    ...
    # Financial Forecasting Settings
    DEFAULT_FORECAST_YEARS: int = int(os.getenv("DEFAULT_FORECAST_YEARS", "5"))
    DEFAULT_USER_ID: str = os.getenv("DEFAULT_USER_ID", "user_123")
    FORECAST_CONFIDENCE_LEVEL: int = int(os.getenv("FORECAST_CONFIDENCE_LEVEL", "80"))
    
    # MCP Server URLs (for production deployment)
    MCP_PERPLEXITY_URL: Optional[str] = os.getenv("MCP_PERPLEXITY_URL")
    MCP_XERO_URL: Optional[str] = os.getenv("MCP_XERO_URL")
    MCP_NOTION_URL: Optional[str] = os.getenv("MCP_NOTION_URL")
